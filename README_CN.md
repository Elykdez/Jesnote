...
- 无延迟打开直接渲染。
- 完整加载并渲染一个包含 6600 万元素 13GB 大小的 JSONL 文件大约耗时 53 秒 (机械硬盘)。
- 上述JSONL文件在默认模式下加载消耗约 7GB 额外内存，在紧凑模式下仅消耗约 500MB 额外内存。
- 以上数据仅为参考，实际速度会因设备硬件而异，尤其是 CPU、内存容量、磁盘/SSD 速度和散热状态。
